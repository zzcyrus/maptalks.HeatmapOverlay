 ...
 */
import { Coordinate, DomUtil, Layer, Point, renderer } from 'maptalks';
...
var Heatmap = function HeatmapClosure() {
  ...
  var _connect = function _connect(scope) {
    var renderer$$1 = scope._renderer;
    var coordinator = scope._coordinator;
    var store = scope._store;

    coordinator.on('renderpartial', renderer$$1.renderPartial, renderer$$1);
    coordinator.on('renderall', renderer$$1.renderAll, renderer$$1);
...
var HeatmapOverlay = function (_maptalks$Layer) {
    _inherits(HeatmapOverlay, _maptalks$Layer);

    function HeatmapOverlay(id, options) {
        _classCallCheck(this, HeatmapOverlay);

        var _this = _possibleConstructorReturn(this, _maptalks$Layer.call(this, id, options));

        _this._el = DomUtil.createEl('div', 'maptalks-heatmap-layer');
        _this.cfg = options;
        _this.cfg.container = _this._el;
        _this.cfg._max = 1;
        _this.cfg._min = 0;
        _this._data = [];
        return _this;
    ...
    HeatmapOverlay.prototype.getData = function getData() {
        return this._data;
    };

    HeatmapOverlay.prototype.getCfg = function getCfg() {
        return this.cfg;
    };

    HeatmapOverlay.prototype.setData = function setData(data) {
        ...
        while (len--) {
            var entry = data[len];
            var latlng = new Coordinate(entry[latField], entry[lngField]);
    ...
    HeatmapOverlay.prototype.addData = function addData(pointOrArray) {
    ...
    HeatmapOverlay.prototype.onConfig = function onConfig(conf) {
        for (var p in conf) {
            if (this.options[p]) {
                return this.redraw();
            }
    ...
    HeatmapOverlay.prototype.redraw = function redraw() {
    ...
    HeatmapOverlay.prototype.isEmpty = function isEmpty() {
    ...
    HeatmapOverlay.prototype.clear = function clear() {
        this._data = [];
        this.redraw();
        this.fire('clear');
        return this;
    ...
    HeatmapOverlay.prototype._getHeatRadius = function _getHeatRadius() {
...
HeatmapOverlay.registerRenderer('canvas', function (_maptalks$renderer$Ca) {
    ...
    _class.prototype.draw = function draw() {
        ...
        this._origin = this._map._pointToContainerPoint(new Point(0, 0));
        this._map.getPanels().frontLayer.appendChild(this._el);
        this._data = layer.getData();
        if (!this._heatmap) {
            this._heatmap = heatmapFactory.create(this.cfg);
        }
        this._draw();
        this.completeRender();
    };
    ...
    _class.prototype._draw = function _draw() {
        if (!this._map) {
            return;
        }
        var point = this._map.offsetPlatform();
    ...
    _class.prototype._update = function _update() {
        ...
        while (len--) {
            ...
            var latlng = new Coordinate(entry.latlng);
            ...
            var point = this._map.coordinateToContainerPoint(latlng);
            var latlngPoint = {
    ...
    _class.prototype.onResize = function onResize() {
        this._origin = this._map._pointToContainerPoint(new Coordinate(0, 0));
        var size = this._map.getSize();
    ...
    _class.prototype.onRemove = function onRemove() {
        this.clearHeatCache();
        this._map().getPanels().frontLayer.removeChild(this._el);
...
typeof console !== 'undefined' && console.log('maptalks.HeatmapOverlay v1.0.0, requires maptalks@^0.40.5.');
